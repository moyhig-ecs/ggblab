...
class AppletInjector2:
    ...
    def open_panel(
        ...
        try:
            ...
            try:
                app.commands.execute('ggblab:create', {
                    'kernelId': kernelId,
    ...
    def open(self, appName: str = 'suite', insertMode: str = 'split-right', socketPath: Optional[str] = None,
        ...
        if GeoGebra is None or not hasattr(GeoGebra, 'init'):
            try:
                ...
                raise RuntimeError(f'Could not import GeoGebra controller from ggblab or ggblab_core2: {e}')
        ...
        try:
            ...
            info = {'kernel_id': getattr(ggb, 'kernel_id', None), 'socketPath': getattr(getattr(ggb, 'comm', None), 'socketPath', None)}
        except Exception as e:
        ...
        opened = self.open_panel(kernelId=info.get('kernel_id'), socketPath=info.get('socketPath'), appName=appName,
                                 commTarget='jupyter.ggblab', insertMode=insertMode)
        if not opened:
        ...
        try:
            if hasattr(ggb, '_run_sync'):
                ...
                def _t2():
                    try:
                        res2['v'] = asyncio.run(ggb.init(appName=appName, use_vscode=False))
                    except Exception as e:
                        exc2['e'] = e
            ...
            raise RuntimeError(f'Failed to initialize GeoGebra instance after injection: {e}')
...
async def setup_comm_and_kernel(ggb) -> dict:
    ...
    return {'kernel_id': ggb.kernel_id, 'socketPath': getattr(ggb.comm, 'socketPath', None)}
...
async def publish_connection_for_vscode(ggb) -> bool:
    ...
    """
    try:
        payload = {'kernelId': getattr(ggb, 'kernel_id', None), 'socketPath': getattr(getattr(ggb, 'comm', None), 'socketPath', None)}
...
def validate_app_name(appName: str) -> str:
    ...
    if appName_norm not in valid_app_names:
        raise ValueError(f"Invalid appName '{appName}'; allowed values: {', '.join(sorted(valid_app_names))}")
    return appName_norm

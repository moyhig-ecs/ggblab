...
v2 (2026-09-10, stage 2 "algebra"): data-in only — no host, no implicit applet lookup, no asyncio; the 3D flag is set
from an XML string (`set_applet_3d_from_xml`).  Everything else is v1 verbatim.
...
def get_applet_3d(force: bool = False) -> Optional[bool]:
    """Return the module-level `_is_applet_3d` flag (True / False / None = unknown).

    v2 (data-in, teacher's ruling 2026-09-07 (i)): there is no detector behind this flag any more — v1 looked for an
    implicit applet (`GeoGebra._instance`, IPython `user_ns["ggb"]`) and awaited `getXML()` here; that hidden state
    and the asyncio dance are gone.  Set the flag from the XML the host handed you: `set_applet_3d_from_xml(xml)`.
    `force` is accepted for v1 call sites (`attach_object2d/3d` pass it) and ignored.
    """
...
def is_applet_3d_from_xml(xml: str) -> Optional[bool]:
    """True/False if the construction document is 3D, None if it cannot be told from the string.

    Pure (data-in): the same predicate v1 evaluated on `getXML()` — the `<geogebra>` root's `app` / `subApp`
    attribute contains "3d" — plus the presence of a `<euclidianView3D>` block.  A bare `<construction>` or an
    element fragment carries neither, so the answer is None (unknown), never a guess.
    """
    ...
    try:
        ...
        root = ET.fromstring(_normalize_exponent(xml.strip()))
    except (ImportError, ValueError, ET.ParseError):
        return None
    if root.tag != "geogebra":
        return None
    app_attr = (root.get("app") or "").lower()
    sub_attr = (root.get("subApp") or "").lower()
    return ("3d" in app_attr) or ("3d" in sub_attr) or (root.find("euclidianView3D") is not None)


def set_applet_3d_from_xml(xml: str) -> Optional[bool]:
    """Set the module flag from a construction document (host's `xml_out` or a .ggb) and return the value."""
    val = is_applet_3d_from_xml(xml)
    set_applet_3d(val)
    return val
...
__all__ = [
    "set_applet_3d",
    "get_applet_3d",
    "is_applet_3d_from_xml",
    "set_applet_3d_from_xml",
    "is_pointlike",

...
- **先生裁定 09-08 昼(逐語)**「『Type を XML の class(静的・schema 列挙)のまま保ち、実行時の種別が要るときは host の read として別列に取る』に替えます。v1 は現場で不整合が起きがちで継続性がない」→ 実装: `ggblab/schema.py: eltype_pattern()`(common.xsd の `elType` pattern = XML 型の閉集合)/ `ConstructionIO.unknown_types(df)`(閉世界検査)/ `ConstructionIO.with_kind(df, kinds)`(実行時種別を label→kind の辞書で受けて `Kind` 列に join・純関数・host には触れない)。試験 +1(計 21)。**残: `kind → getObjectType` を C1 の read 動詞 #8 として allow-list に載せるか(先生)**。それまで `Kind` は Playwright 捕捉のような外部読みでしか埋まらない。

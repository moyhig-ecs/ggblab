...
try:
    ...
    if _sanitized != _orig_argv:
        logging.getLogger(__name__).debug("proxy_launcher: sanitized argv from %r to %r", _orig_argv, _sanitized)
        sys.argv = _sanitized
except Exception:
...
def main():
    ...
    logging.getLogger("tornado").debug("proxy_launcher: GGBLAB_ENABLE=%r", os.environ.get("GGBLAB_ENABLE"))

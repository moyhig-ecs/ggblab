...
const WebSocketServer = _ws.WebSocketServer || _ws.Server;
const WebSocket = _ws.WebSocket || _ws;
function activate(context) {
    ...
    try {
        ...
        let wsPort = null;
        const clientMetaBySocket = new Map();
        const kernelClients = new Map(); // kernelId -> Set<ws>
        const webviewClients = new Set();
        // persistent pending queues (in-memory) to hold messages when no webview is present
        const pendingQueues = new Map(); // kernelId -> [jsonStr]
        const pendingBroadcasts = [];
        const PENDING_LIMIT = 2000;
        const wss = new WebSocketServer({ host: '127.0.0.1', port: 0 });
        try {
            channel.appendLine('ws-broker: WebSocketServer created');
        }
        catch (e) { }
        try {
            wss.on('error', (err) => { channel.appendLine(`ws-broker: server error: ${(err && err.stack) ? err.stack : String(err)}`); });
        }
        catch (e) { }
        ...
        const heartbeat = setInterval(() => {
            try {
                wss.clients.forEach((c) => {
                    try {
                        const meta = clientMetaBySocket.get(c) || {};
                        const now = Date.now();
                        const last = meta.lastSeen || (c.isAlive ? now : 0);
                        const missed = Math.floor((now - last) / heartbeatInterval);
                        if (missed >= maxMissed) {
                            try {
                                channel.appendLine(`ws-broker: heartbeat terminate client missed=${missed}`);
                            }
                            catch (e) { }
                            try {
                                c.terminate();
        ...
        wss.on('connection', (ws, req) => {
            ...
            try {
                ws.on('pong', () => { try {
                    ws.isAlive = true;
                    const m = clientMetaBySocket.get(ws) || {};
                    m.lastSeen = Date.now();
                    clientMetaBySocket.set(ws, m);
                }
                catch (e) { } });
            }
            catch (e) { }
            try {
                const sock = ws._socket || ws.socket || null;
            ...
            try {
                clientMetaBySocket.set(ws, { lastSeen: Date.now(), outQueue: [] });
            }
            catch (e) { }
            // log transport errors
            try {
                ws.on('error', (err) => { channel.appendLine(`ws-broker: ws error: ${(err && err.stack) ? err.stack : String(err)}`); });
            }
            catch (e) { }
            let authenticated = false;
            let meta = { kind: 'unknown', kernelId: null };
            ws.on('message', (data) => {
                try {
                    // Log raw incoming message for debugging
                    try {
                        channel.appendLine(`ws-broker: raw-msg type=${typeof data} len=${data && (data.length || JSON.stringify(data).length)}`);
                    }
                    catch (e) { }
                    try {
                    ...
                    try {
                        obj = typeof text === 'string' ? JSON.parse(text) : data;
                    }
                    catch (e) {
                        obj = data;
                    }
                    ...
                    try {
                        const preview = (typeof text === 'string' ? text : JSON.stringify(obj)).slice(0, 200);
                        channel.appendLine(`ws-broker: raw-json-preview ${preview}`);
                    }
                    catch (e) { }
                    ...
                    if (!authenticated) {
                        if (obj && obj.type === 'hello' && obj.token === wsToken) {
                            ...
                            if (meta.kind === 'kernel' && meta.kernelId) {
                                ...
                                if (!s) {
                                    s = new Set();
                                    kernelClients.set(meta.kernelId, s);
                                }
                                s.add(ws);
                            }
                            ...
                            try {
                                const sm = clientMetaBySocket.get(ws) || {};
                                if (Array.isArray(sm.outQueue) && sm.outQueue.length) {
                                    for (const m of sm.outQueue) {
                                        try {
                                            if (ws.readyState === ws.OPEN)
                                                ws.send(m);
                                        }
                                        catch (e) {
                                            channel.appendLine(`ws-broker: flush-send failed ${String(e)}`);
                                        }
                                    }
                                    sm.outQueue = [];
                                    clientMetaBySocket.set(ws, sm);
                            ...
                            try {
                                    ...
                                    if (pendingBroadcasts.length > 0) {
                                        for (const msg of pendingBroadcasts) {
                                            for (const vw of webviewClients) {
                                                try {
                                                    if (vw.readyState === vw.OPEN)
                                                        vw.send(msg);
                                                }
                                                catch (e) {
                                                    channel.appendLine(`ws-broker: flush-pending-broadcast failed ${String(e)}`);
                                                }
                                            }
                                        }
                                        ...
                                        for (const msg of arr) {
                                            for (const vw of webviewClients) {
                                                try {
                                                    ...
                                                    channel.appendLine(`ws-broker: flush-pending-kid failed ${String(e)}`);
                                                }
                                            }
                                        }
                                ...
                                channel.appendLine(`ws-broker: pending flush error ${String(e)}`);
                            }
                            return;
                        }
                        else {
                    ...
                    if (authenticated && obj && obj.type === 'set-kernel-id' && obj.kernelId) {
                        try {
                            ...
                            if (!s) {
                                s = new Set();
                                kernelClients.set(kid, s);
                            }
                            s.add(ws);
                            channel.appendLine(`ws-broker: kernel id set post-auth kernel=${kid}`);
                        }
                        catch (e) {
                            channel.appendLine(`ws-broker: set-kernel-id failed: ${String(e)}`);
                        }
                        return;
                    }
                                ...
                                for (const c of set) {
                                    try {
                                        if (c.readyState === c.OPEN) {
                                            ...
                                            try {
                                                const cm = clientMetaBySocket.get(c) || {};
                                                cm.outQueue = cm.outQueue || [];
                                                cm.outQueue.push(asStr);
                                                clientMetaBySocket.set(c, cm);
                                            }
                                            catch (e) { }
                                        }
                                    }
                                    catch (e) {
                                        channel.appendLine(`ws-broker: send->kernel failed ${(e && e.stack) ? e.stack : String(e)}`);
                                    }
                                }
                            }
                        }
                            ...
                            if (webviewClients.size === 0) {
                                ...
                                for (const c of webviewClients) {
                                    try {
                                        ...
                                        channel.appendLine(`ws-broker: send->webview failed ${(e && e.stack) ? e.stack : String(e)}`);
                                    }
                                }
                            }
                        }
                        ...
                        if (set)
                            for (const c of set)
                                try {
                                    ...
                                    channel.appendLine(`ws-broker: fallback-send failed ${(e && e.stack) ? e.stack : String(e)}`);
                                }
                    }
                }
                catch (e) {
                ...
                try {
                    ...
                    if (m && m.kind === 'kernel' && m.kernelId) {
                        ...
                        if (s) {
                            s.delete(ws);
                            if (s.size === 0)
                                kernelClients.delete(m.kernelId);
                        }
                    }
                    if (m && m.kind === 'webview')
                    ...
                    try {
                        channel.appendLine(`ws-broker: connection closed code=${code} reason=${String(reason)}`);
                    }
                    catch (e) { }
                }
                catch (e) {
                    try {
                        channel.appendLine(`ws-broker: connection closed handler error: ${e}`);
                    }
                    catch (er) { }
                }
            });
        ...
        // requests itself.
        let savedBaseUrl = undefined;
        let savedToken = undefined;
        const kernelConnections = new Map();
        async function ensureKernelTarget(kernelObj, targetName) {
            try {
                if (!kernelObj || typeof kernelObj.requestExecute !== 'function')
                    return false;
                ...
                try {
                    const fut = kernelObj.requestExecute({ code });
                    if (fut && typeof fut.onIOPub === 'function') {
                        try {
                            fut.onIOPub((m) => channel.appendLine(`ensureKernelTarget iopub: ${JSON.stringify(m)}`));
                        }
                        catch (e) { }
                    }
        ...
        const disposable = vscode.commands.registerCommand('ggblab.openWebview', () => {
            ...
            try {
                ...
                const panel = vscode.window.createWebviewPanel('ggblabView', 'GGBlab GeoGebra', vscode.ViewColumn.One, {
                    enableScripts: true,
                    ...
                    try {
                        ...
                        (async () => {
                            ...
                            try {
                                ...
                                if (tokenInput)
                                    opts.headers['Authorization'] = `Token ${tokenInput}`;
                                ...
                                req.on('error', (err) => channel.appendLine(`proxy-initial-error: ${err}`));
                                req.end();
                            }
                            catch (err) {
                ...
                panel.webview.onDidReceiveMessage(async (msg) => {
                    try {
                        ...
                        if (msg.type === 'kernel-ws-open') {
                            ...
                            if (!savedBaseUrl) {
                                try {
                                    panel.webview.postMessage({ type: 'kernel-ws-opened', id, error: 'no-jupyter-settings' });
                                }
                                catch (e) { }
                            ...
                            try {
                                const serverSettings = services_1.ServerConnection.makeSettings({ baseUrl: savedBaseUrl, wsUrl: savedBaseUrl.replace(/^http/, 'ws'), token: savedToken });
                                const km = new services_1.KernelManager({ serverSettings });
                                let promise;
                                if (msg.kernelId) {
                                    try {
                                ...
                                promise.then(async (kernelConn) => {
                                    ...
                                    try {
                                        ...
                                        if (k.iopubMessage && typeof k.iopubMessage.connect === 'function') {
                                            k.iopubMessage.connect((sender, msg) => {
                                                try {
                                    ...
                                    try {
                                        panel.webview.postMessage({ type: 'kernel-ws-opened', id, error: String(err) });
                                    }
                                    catch (e) { }
                        ...
                        if (msg.type === 'kernel-probe') {
                            ...
                            if (!savedBaseUrl) {
                                try {
                                    panel.webview.postMessage({ type: 'kernel-probe-response', id, error: 'no-jupyter-settings' });
                                }
                                catch (e) { }
                            ...
                            try {
                                ...
                                const opts = { method: 'GET', headers: {} };
                                if (savedToken)
                                    opts.headers['Authorization'] = `Token ${savedToken}`;
                                const req = client.request(u, opts, (res) => {
                                    let body = '';
                                    res.setEncoding('utf8');
                                    res.on('data', (chunk) => (body += chunk));
                                    res.on('end', () => {
                                        try {
                                            panel.webview.postMessage({ type: 'kernel-probe-response', id, status: res.statusCode, body });
                                        }
                                        catch (e) { }
                                ...
                                req.on('error', (err) => { try {
                                    panel.webview.postMessage({ type: 'kernel-probe-response', id, error: String(err) });
                                }
                                catch (e) { } ; channel.appendLine(`kernel-probe-error: ${err}`); });
                                req.end();
                            }
                            catch (err) {
                                try {
                        ...
                        if (msg.type === 'kernel-start') {
                            ...
                            if (!savedBaseUrl) {
                                try {
                                    panel.webview.postMessage({ type: 'kernel-start-response', id, error: 'no-jupyter-settings' });
                                }
                                catch (e) { }
                            ...
                            try {
                                ...
                                const opts = { method: 'POST', headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(bodyPayload) } };
                                if (savedToken)
                                    opts.headers['Authorization'] = `Token ${savedToken}`;
                                const req = client.request(u, opts, (res) => {
                                    let body = '';
                                    ...
                                    res.on('end', async () => {
                                        try {
                                            panel.webview.postMessage({ type: 'kernel-start-response', id, status: res.statusCode, body });
                                        }
                                        catch (e) { }
                                            ...
                                            try {
                                                ...
                                                if (restKernelId && savedBaseUrl) {
                                                    ...
                                                    try {
                                                        ...
                                                        if (restKernelId)
                                                            kernelConnections.set(restKernelId, kernelConn);
                                                        kernelConnections.set(kid, kernelConn);
                                                        // Notify webview and attach iopub comm forwarding
                                                        try {
                                                            panel.webview.postMessage({ type: 'kernel-ws-opened', id: id + '-auto', status: 'ok', kernelId: kid });
                                                        }
                                                        catch (e) { }
                                ...
                                req.on('error', (err) => { try {
                                    panel.webview.postMessage({ type: 'kernel-start-response', id, error: String(err) });
                                }
                                catch (e) { } ; channel.appendLine(`kernel-start-error: ${err}`); });
                        ...
                        if (msg.type === 'comm-open') {
                            ...
                            const ensureAndSend = async (kernelObj) => {
                                try {
                                    const kv = kernelObj;
                                    if (kv.createComm) {
                                        try {
                                            ...
                                            if (comm && typeof comm.open === 'function') {
                                                comm.open(data);
                                                try {
                                                    panel.webview.postMessage({ type: 'comm-open-response', id, status: 'ok' });
                                                }
                                                catch (e) { }
                                                return true;
                                            }
                                            ...
                                            if (msg.includes('already') || msg.includes('already created')) {
                                                try {
                                                    panel.webview.postMessage({ type: 'comm-open-response', id, status: 'ok', note: 'already-created' });
                                                }
                                                catch (ee) { }
                                                return true;
                                            }
                                            try {
                                                panel.webview.postMessage({ type: 'comm-open-response', id, error: String(e) });
                                            }
                                            catch (ee) { }
                                        }
                                    }
                                    ...
                                    try {
                                        panel.webview.postMessage({ type: 'comm-open-response', id, error: 'no-comm-api' });
                                    }
                                    catch (e) { }
                                    return false;
                                }
                                    ...
                                    try {
                                        panel.webview.postMessage({ type: 'comm-open-response', id, error: String(err) });
                                    }
                                    catch (e) { }
                            ...
                            if (!k && kid && savedBaseUrl) {
                                try {
                                    ...
                                    try {
                                        ...
                                        const km2 = new services_1.KernelManager({ serverSettings });
                                        const sm = new services_1.SessionManager({ serverSettings, kernelManager: km2 });
                                        const session = await sm.startNew({ path: `ggblab-${kid}`, name: '', type: 'terminal', kernel: { id: kid, name: msg.kernelName || 'python3' } });
                            ...
                            if (!k) {
                                try {
                                    panel.webview.postMessage({ type: 'comm-open-response', id, error: 'no-kernel' });
                                }
                                catch (e) { }
                        ...
                        if (msg.type === 'comm-msg') {
                            ...
                            const trySend = async (kernelObj) => {
                                try {
                                    ...
                                    try {
                                        ...
                                        channel.appendLine(`comm-msg send inner error: ${innerErr && innerErr.stack ? innerErr.stack : String(innerErr)}`);
                                        ...
                                        try {
                                            ...
                                            channel.appendLine(`comm-msg recovery failed: ${recreateErr && recreateErr.stack ? recreateErr.stack : String(recreateErr)}`);
                                        }
                                    }
                                    if (sent) {
                                        try {
                                            panel.webview.postMessage({ type: 'comm-msg-response', id, status: 'sent' });
                                        }
                                        catch (e) { }
                                    }
                                    else {
                                        try {
                                            panel.webview.postMessage({ type: 'comm-msg-response', id, error: 'no-comm-object' });
                                        }
                                        catch (e) { }
                                    }
                                }
                                    ...
                                    try {
                                        panel.webview.postMessage({ type: 'comm-msg-response', id, error: String(err), stack: err && err.stack ? err.stack : '' });
                                    }
                                    catch (e) { }
                                    channel.appendLine(`comm-msg-exception: ${err && err.stack ? err.stack : String(err)}`);
                                }
                            };
                            if (!k && kid && savedBaseUrl) {
                                try {
                            ...
                            if (!k) {
                                try {
                                    panel.webview.postMessage({ type: 'comm-msg-response', id, error: 'no-kernel' });
                                }
                                catch (e) { }
                        ...
                        if (msg.type === 'comm-close') {
                            ...
                            if (!k) {
                                try {
                                    panel.webview.postMessage({ type: 'comm-close-response', id, error: 'no-kernel' });
                                }
                                catch (e) { }
                            ...
                            try {
                                ...
                                if (kv._comms && kv._comms[commId] && typeof kv._comms[commId].close === 'function') {
                                    ...
                                    try {
                                        panel.webview.postMessage({ type: 'comm-close-response', id, status: 'closed' });
                                    }
                                    catch (e) { }
                                }
                                else {
                                    try {
                                        panel.webview.postMessage({ type: 'comm-close-response', id, error: 'no-comm-object' });
                                    }
                                    catch (e) { }
                                }
                            }
                                ...
                                try {
                                    panel.webview.postMessage({ type: 'comm-close-response', id, error: String(err) });
                                }
                                catch (e) { }
                        ...
                        if (msg.type === 'connect-request' && msg.url) {
                            ...
                            try {
                                ...
                                req.on('error', (err) => {
                                    try {
                                        panel.webview.postMessage({ type: 'proxy-response', id, error: String(err) });
                                    }
                                    catch (e) { }
                        ...
                        try {
                            ...
                            if (kc._comms === undefined) {
                                ...
                                if (kc._comms instanceof Map) {
                                    ...
                                    for (const nk of own) {
                                        try {
                                            channel.appendLine(`    comm key: ${nk} typeof:${typeof kc._comms[nk]}`);
                                        }
                                        catch (e) { }
                                        commCount++;
                                    }
...
function getWebviewContent(scriptUri, nonce, cspSource, wsPort, wsToken) {

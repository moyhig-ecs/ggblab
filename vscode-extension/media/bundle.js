...
(() => {
  ...
  var GGAComponent = () => {
    return /* @__PURE__ */ import_react.default.createElement("div", { style: { padding: 8 } }, "GGBlab placeholder component");
  };
  function App() {
    const [baseUrl, setBaseUrl] = import_react.default.useState("http://localhost:8888");
    const [token, setToken] = import_react.default.useState("");
    const [connected, setConnected] = import_react.default.useState(false);
    const [kernels, setKernels] = import_react.default.useState([]);
    const [kernelName, setKernelName] = import_react.default.useState("python3");
    const [comms, setComms] = import_react.default.useState([]);
    const [commTarget, setCommTarget] = import_react.default.useState("ggblab");
    const [commId, setCommId] = import_react.default.useState("");
    const [commMsg, setCommMsg] = import_react.default.useState("hello");
    const [currentKernelId, setCurrentKernelId] = import_react.default.useState(null);
    const lastProxyId = import_react.default.useRef(null);
    const [localLogs, setLocalLogs] = import_react.default.useState([]);
    const appendLocalLog = (msg) => {
      setLocalLogs((prev) => [...prev.slice(-200), `${(/* @__PURE__ */ new Date()).toISOString()} ${msg}`]);
    };
    const vsRef = import_react.default.useRef(null);
    const getVs = () => {
      if (vsRef.current)
        return vsRef.current;
      try {
        const a = window.acquireVsCodeApi && window.acquireVsCodeApi();
        vsRef.current = a;
        return a;
      } catch (e) {
        return null;
      ...
      try {
        ...
        try {
          ...
          if (vs && vs.postMessage)
            vs.postMessage({ type: "webview-log", level: "log", args: ["connect-start", baseUrl] });
        } catch (e) {
        }
        ...
        try {
          ...
          if (vs && vs.postMessage) {
            ...
            try {
              vs.postMessage({ type: "webview-log", level: "log", args: ["connect-request-send", apiUrl] });
            } catch (e) {
            }
            try {
              vs.postMessage({ type: "webview-log", level: "log", args: ["connect-request-generic", apiUrl, reqId] });
            } catch (e) {
            }
            try {
              vs.postMessage({ type: "connect-request", id: reqId, url: apiUrl, headers });
              appendLocalLog("posted connect-request");
            } catch (e) {
              appendLocalLog("failed posting connect-request " + String(e));
            }
            return;
          } else {
          ...
          try {
            const vs2 = getVs();
            if (vs2 && vs2.postMessage)
              vs2.postMessage({ type: "webview-log", level: "log", args: ["fetch-status", resp.status] });
          } catch (e) {
          }
          const text = await resp.text().catch(() => "<no-body>");
          try {
            const vs3 = getVs();
            if (vs3 && vs3.postMessage)
              vs3.postMessage({ type: "webview-log", level: "log", args: ["fetch-body", text.slice(0, 200)] });
          } catch (e) {
          }
          if (resp.ok) {
            setConnected(true);
            try {
              const vs4 = getVs();
              if (vs4 && vs4.postMessage)
                vs4.postMessage({ type: "webview-log", level: "log", args: ["connect-success", resp.status] });
            } catch (e) {
            }
          } else {
            setConnected(false);
            try {
              const vs5 = getVs();
              if (vs5 && vs5.postMessage)
                vs5.postMessage({ type: "webview-log", level: "error", args: ["connect-failed", resp.status] });
            } catch (e) {
            }
          }
        } catch (fetchErr) {
          try {
            const vs6 = getVs();
            if (vs6 && vs6.postMessage)
              vs6.postMessage({ type: "webview-log", level: "error", args: ["fetch-exception", String(fetchErr)] });
          } catch (e) {
          }
          setConnected(false);
        }
        ...
        console.error(e);
        try {
          const vs = window.acquireVsCodeApi && window.acquireVsCodeApi();
          if (vs && vs.postMessage)
            vs.postMessage({ type: "webview-log", level: "error", args: ["connect-exception", String(e)] });
        } catch (err) {
        }
        setConnected(false);
      }
    ...
    const probeKernels = () => {
      ...
      try {
        ...
        if (vs && vs.postMessage) {
          ...
          try {
            vs.postMessage({ type: "webview-log", level: "log", args: ["kernel-probe-send"] });
          } catch (e) {
          }
        ...
        appendLocalLog("kernel-probe exception " + String(e));
      }
    };
    const startKernel = (name) => {
      ...
      try {
        ...
        if (vs && vs.postMessage) {
          const id = String(Date.now());
          try {
            vs.postMessage({ type: "webview-log", level: "log", args: ["kernel-start-send", kn] });
          } catch (e) {
          }
        ...
        appendLocalLog("kernel-start exception " + String(e));
      }
    };
    import_react.default.useEffect(() => {
      ...
      const tryAttach = () => {
        ...
        console.log = (...args) => {
          try {
            a.postMessage({ type: "webview-log", level: "log", args });
          } catch (e) {
          }
          try {
            appendLocalLog((args || []).map((x) => typeof x === "string" ? x : JSON.stringify(x)).join(" "));
          } catch (e) {
          }
          origLog(...args);
        };
        console.error = (...args) => {
          try {
            a.postMessage({ type: "webview-log", level: "error", args });
          } catch (e) {
          }
          origError(...args);
        };
        try {
          a.postMessage({ type: "webview-ready" });
        } catch (e) {
        }
        return true;
      };
      ...
      const handler = (ev) => {
        ...
        if (msg.type === "extension-ping") {
          try {
            const vs = getVs();
            if (vs && vs.postMessage)
              vs.postMessage({ type: "webview-pong" });
          } catch (e) {
          }
          return;
        }
        ...
        if (msg.type === "ws-broker") {
          ...
          try {
            window.ggblabWsBrokerUrl = msg.url;
            window.ggblabWsBrokerToken = msg.token || "";
          ...
          if (matches) {
            if (msg.error) {
              try {
                const vs = getVs();
                if (vs && vs.postMessage)
                  vs.postMessage({ type: "webview-log", level: "error", args: ["proxy-error", msg.error] });
              } catch (e) {
              }
              setConnected(false);
              return;
            ...
            try {
              const vs = getVs();
              if (vs && vs.postMessage)
                vs.postMessage({ type: "webview-log", level: "log", args: ["proxy-status", msg.status] });
            } catch (e) {
            }
            try {
              const vs = getVs();
              if (vs && vs.postMessage)
                vs.postMessage({ type: "webview-log", level: "log", args: ["proxy-body", (msg.body || "").slice(0, 200)] });
            } catch (e) {
            }
            ...
            if (msg.status && msg.status >= 200 && msg.status < 300) {
              ...
              try {
                const vs = window.acquireVsCodeApi && window.acquireVsCodeApi();
                if (vs && vs.postMessage)
                  vs.postMessage({ type: "webview-log", level: "log", args: ["connect-success", msg.status] });
              } catch (e) {
              }
            } else {
              setConnected(false);
              try {
                const vs = window.acquireVsCodeApi && window.acquireVsCodeApi();
                if (vs && vs.postMessage)
                  vs.postMessage({ type: "webview-log", level: "error", args: ["connect-failed", msg.status] });
              } catch (e) {
              }
            }
          }
        ...
        if (msg.type === "kernel-start-response") {
          ...
          if (msg.error)
            appendLocalLog(`kernel-start-error: ${msg.error}`);
          else {
    ...
    import_react.default.useEffect(() => {
      ...
      const handler = () => {
        try {
          const vs = getVs();
          if (vs && vs.postMessage)
            vs.postMessage({ type: "webview-log", level: "log", args: ["connect-clicked-dom"] });
        } catch (e) {
        }
      };
    ...
    return /* @__PURE__ */ import_react.default.createElement("div", { style: { height: "100%", display: "flex", flexDirection: "column" } }, /* @__PURE__ */ import_react.default.createElement("div", { style: { padding: 8, background: "#f3f3f3" } }, /* @__PURE__ */ import_react.default.createElement("label", null, "Jupyter base URL: "), /* @__PURE__ */ import_react.default.createElement("input", { value: baseUrl, onChange: (e) => setBaseUrl(e.target.value), style: { width: 300 } }), /* @__PURE__ */ import_react.default.createElement("label", { style: { marginLeft: 8 } }, "Token: "), /* @__PURE__ */ import_react.default.createElement("input", { value: token, onChange: (e) => setToken(e.target.value), style: { width: 200 } }), /* @__PURE__ */ import_react.default.createElement("button", { id: "ggblab-connect-btn", onClick: () => {
      try {
        const vs = getVs();
        if (vs && vs.postMessage)
          vs.postMessage({ type: "webview-log", level: "log", args: ["connect-clicked"] });
      } catch (e) {
      }
      ;
      connect();
    }, style: { marginLeft: 8 } }, "Connect"), /* @__PURE__ */ import_react.default.createElement("button", { onClick: () => {
      appendLocalLog("force-connect clicked");
      setConnected(true);
    }, style: { marginLeft: 8 } }, "Force Connected"), /* @__PURE__ */ import_react.default.createElement("label", { style: { marginLeft: 12 } }, "Kernel: "), /* @__PURE__ */ import_react.default.createElement("input", { value: kernelName, onChange: (e) => setKernelName(e.target.value), style: { width: 140, marginLeft: 6 } }), /* @__PURE__ */ import_react.default.createElement("button", { onClick: () => probeKernels(), style: { marginLeft: 8 } }, "Probe Kernels"), /* @__PURE__ */ import_react.default.createElement("button", { onClick: () => startKernel(), style: { marginLeft: 8 } }, "Start Kernel"), /* @__PURE__ */ import_react.default.createElement("label", { style: { marginLeft: 12 } }, "Comm target:"), /* @__PURE__ */ import_react.default.createElement("input", { value: commTarget, onChange: (e) => setCommTarget(e.target.value), style: { width: 120, marginLeft: 6 } }), /* @__PURE__ */ import_react.default.createElement("input", { placeholder: "commId (opt)", value: commId, onChange: (e) => setCommId(e.target.value), style: { width: 160, marginLeft: 6 } }), /* @__PURE__ */ import_react.default.createElement("button", { onClick: () => {
      try {
        const vs = getVs();
        if (vs && vs.postMessage) {
          const id = String(Date.now());
          const kid = currentKernelId || kernels[0] && kernels[0].id || void 0;
        ...
        appendLocalLog("comm-open error " + String(e));
      }
    }, style: { marginLeft: 8 } }, "Open Comm"), /* @__PURE__ */ import_react.default.createElement("input", { value: commMsg, onChange: (e) => setCommMsg(e.target.value), style: { width: 220, marginLeft: 8 } }), /* @__PURE__ */ import_react.default.createElement("button", { onClick: () => {
      try {
        const vs = getVs();
        if (vs && vs.postMessage) {
          const id = String(Date.now());
          ...
          vs.postMessage({ type: "comm-msg", id, kernelId: kid, commId, data: { text: commMsg } });
          appendLocalLog("comm-msg sent");
        }
      } catch (e) {
        appendLocalLog("comm-msg error " + String(e));
      }
    }, style: { marginLeft: 8 } }, "Send Comm Msg"), /* @__PURE__ */ import_react.default.createElement("span", { style: { marginLeft: 12 } }, connected ? "Connected" : "Disconnected")), /* @__PURE__ */ import_react.default.createElement("div", { style: { height: 140, overflow: "auto", background: "#111", color: "#dcdcdc", fontFamily: "monospace", padding: 8 } }, /* @__PURE__ */ import_react.default.createElement("div", { style: { fontSize: 12, marginBottom: 6, color: "#9cdcfe" } }, "Debug log (local)"), /* @__PURE__ */ import_react.default.createElement("pre", { style: { margin: 0, whiteSpace: "pre-wrap" } }, localLogs.join("\n"))), /* @__PURE__ */ import_react.default.createElement("div", { style: { flex: 1 } }, /* @__PURE__ */ import_react.default.createElement("div", { style: { padding: 8 } }, /* @__PURE__ */ import_react.default.createElement("div", { style: { marginBottom: 8 } }, /* @__PURE__ */ import_react.default.createElement("strong", null, "Available kernels:"), " ", kernels.length), /* @__PURE__ */ import_react.default.createElement("pre", { style: { background: "#f6f6f6", padding: 8, height: 240, overflow: "auto" } }, JSON.stringify(kernels, null, 2))), /* @__PURE__ */ import_react.default.createElement(GGAComponent, { kernelId: "", commTarget: "", socketPath: "", wsPort: 8888, appName: "suite" })));
  }

...
mod_path = Path(__file__).resolve().parents[1] / 'ggblab_extra' / 'construction_parser.py'
spec = importlib.util.spec_from_file_location('repro_construction_parser', str(mod_path))
...
sel = parser.df.select([pl.col("Name"), pl.col("Type"), pl.col("Command"), pl.col("Layer")])

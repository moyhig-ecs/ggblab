...
static = cls["exact"] + cls["ws"] + cls["quote"] + cls["paren"] + cls["num"] + cls["mismatch"]
print(f"  static (comparable) lines = {static}; exact {cls['exact']} / ws {cls['ws']} / quote {cls['quote']} / paren {cls['paren']} / num {cls['num']} / mismatch {cls['mismatch']}")
for k in ("mismatch", "quote", "num", "paren", "ws", "error", "nosend", "dynamic"):
...
# positive control
cls_m, _ = run(True)
multi = sum(1 for d in detail if d["class"] in ("exact", "ws", "quote", "paren", "num") and d.get("kind") == "command" and len(parse_statement(d["body"]).args) >= 2
            and len({render_arg(a) for a in parse_statement(d["body"]).args}) >= 2)
lost = (cls["exact"] + cls["ws"] + cls["quote"] + cls["paren"] + cls["num"]) - (cls_m["exact"] + cls_m["ws"] + cls_m["quote"] + cls_m["paren"] + cls_m["num"])
...
print(("⭕ GATE PASS (every static line is GeoGebra-equal under the named normalisations)" if verdict else "⛔ GATE FAIL") + f"  — byte-different but GeoGebra-equal: ws {cls['ws']} / paren {cls['paren']} / num {cls['num']} / quote {cls['quote']} (see report; XML-level equality is gate #2)")

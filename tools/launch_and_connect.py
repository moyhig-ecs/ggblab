...
def main():
    parser = argparse.ArgumentParser(
        description="Launch proxy kernel and open jupyter console connected to it"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--kernel-cmd",
        help="Full kernel command to run for the kernel process (string). Use {connection_file} to place the connection file.",
    )
    parser.add_argument(
        "--install-kernelspec",
        action="store_true",
        help="Install a kernelspec that launches the proxy kernel",
    )
    parser.add_argument(
        "--kernelspec-name",
        default="ggblab-proxy",
        help="Name for the installed kernelspec",
    )
    parser.add_argument(
        "--kernelspec-display-name",
        default="ggblab Proxy Kernel",
    ...
    proxy_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "proxy_repl.py")
    )
    ...
    def _install_kernelspec(proxy_path, name, display_name):
        if KernelSpecManager is None:
            print(
                "jupyter_client not available; install jupyter_client to register kernelspec",
                file=sys.stderr,
            )
            return 2
    ...
    if args.install_kernelspec:
        rc = _install_kernelspec(
            proxy_path, args.kernelspec_name, args.kernelspec_display_name
        )
        sys.exit(rc)

    # create temporary connection file path (the file will be created by the kernel)
    tf = tempfile.NamedTemporaryFile(
        prefix="ggblab-connection-", suffix=".json", delete=False
    )
    conn_path = tf.name
    tf.close()
    ...
    try:
        ...
        for _ in range(50):
            ...
            if kp.poll() is not None:
                logging.error(
                    "Kernel process exited prematurely with code %s", kp.returncode
                )
                sys.exit(1)
            time.sleep(0.1)
        else:

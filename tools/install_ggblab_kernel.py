...
def main():
    parser = argparse.ArgumentParser(description='Install/uninstall ggblab Jupyter kernel')
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument('--install', action='store_true', help='Install the kernelspec (user scope)')
    group.add_argument('--remove', action='store_true', help='Uninstall the kernelspec')
    parser.add_argument('--launch-path', help='Path to ggblab_kernel_launch.py (defaults to tools/ggblab_kernel_launch.py in repo)')

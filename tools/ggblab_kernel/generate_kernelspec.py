...
def make_kernelspec(dest_dir: str, display_name: str = 'ggblab (dev)', language: str = 'python'):
...
def install_kernelspec(dest_dir: str, user: bool = True, replace: bool = False) -> int:
    ...
    try:
        ...
        if not jupyter:
            print('Could not find `jupyter` command to install kernelspec', file=sys.stderr)
            return 2
    ...
    if proc.returncode != 0:
        print('kernelspec install failed (exit {}):'.format(proc.returncode), file=sys.stderr)
        if proc.stdout:
...
def auto_install_kernelspec(kernel_dirname: str, kernel_json_path: str, prefer_user: bool = True, replace: bool = False) -> int:
    ...
    if prefer_user:
        for c in candidates:
            try:
                if os.path.commonpath([os.path.abspath(c), home]) == os.path.abspath(home):
                    chosen = c
                    break
            except Exception:
                continue
    ...
    if chosen is None:
        print('No suitable kernels directory found to install kernelspec', file=sys.stderr)
        return 2

    dest = os.path.join(chosen, kernel_dirname)
    if os.path.exists(dest):
        if not replace:
            print(f'Kernelspec directory already exists: {dest} (use --replace to overwrite)', file=sys.stderr)
            return 3
        # remove and recreate
        try:
            shutil.rmtree(dest)
...
def main():
    ...
    parser.add_argument('--dest', required=True, help='Destination directory to write the kernelspec (will be created)')
    parser.add_argument('--display-name', default='ggblab (dev)', help='Display name for the kernelspec')
    parser.add_argument('--language', default='python', help='Language name')
    parser.add_argument('--install', action='store_true', help='Install the kernelspec after generating (uses python -m jupyter)')
    parser.add_argument('--user', action='store_true', help='Pass --user to `jupyter kernelspec install`')
    parser.add_argument('--replace', action='store_true', help='Pass --replace to `jupyter kernelspec install`')
    parser.add_argument('--auto-install', action='store_true', help='Detect Jupyter kernels directories and write the kernelspec directly')
    parser.add_argument('--name', help='Kernelspec directory name to install as when using --auto-install (default: ggblab)')
    ...
    if args.auto_install:
        ...
        rc = auto_install_kernelspec(name, kernel_json, prefer_user=args.user, replace=args.replace)
        if rc != 0:
            raise SystemExit(rc)

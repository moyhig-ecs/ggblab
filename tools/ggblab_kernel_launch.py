...
def main():
    parser = argparse.ArgumentParser(description='Launch a kernel subprocess via jupyter_client, making the repo importable')
    ...
    kernel_cmd = [sys.executable, '-m', 'ipykernel_launcher', '-f', args.connection_file]
    ...
    try:
        ...
        print('Failed to start kernel subprocess via jupyter_client:', e, file=sys.stderr)
        sys.exit(1)

    print('Kernel subprocess started (pid={})'.format(getattr(km, 'kernel_pid', getattr(km, 'kernel', None))))

  ...
  async function callRemoteSocketSend(message: string): Promise<void> {
    try {
      ...
      const doSend = async () => {
        ...
        // If `kernel2` is not available (e.g., running outside full JupyterLab),
        ...
        if (res.kernel2 && typeof res.kernel2.requestExecute === 'function') {
          ...
          // Fallback: try direct browser WebSocket (only works for wsUrl)
          if (!socketPath) {
            try {
              await new Promise<void>((resolve, reject) => {
                ...
                ws.onerror = (ev: any) => {
                  clearTimeout(timer);
                  dbg && dbg('Browser WebSocket fallback error', ev);
                  try {
                    ws.close();
                  } catch (_) {}
                  resolve();
              ...
              dbg && dbg('Browser WebSocket fallback failed', e);
            }
          } else {
            dbg && dbg('Cannot send via unix socket: no helper kernel available');
          }
        }

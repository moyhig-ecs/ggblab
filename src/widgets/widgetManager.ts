      ...
      commOp.onMsg = async (m: any) => {
        ...
        try {
          ...
          if (rmsg) {
            ...
            try { await opts.callRemoteSocketSend(rmsg); } catch (e) { dbg('callRemoteSocketSend failed', e); }
          }
        } catch (e) {
...
export async function registerGlobalGGBlabCommTargets(app?: any): Promise<() => void> {
  ...
  const dbg = (..._args: any[]) => {
    if (!ENABLE_RUNNING_CHANGED) { return; }
    console.debug(..._args);
  };
  ...
  const registerKernel = (k: any) => {
    ...
    if (!id) return;
    if (registry.has(id)) { dbg('Already registered jupyter.ggblab for kernel', id); return; }
    try {
      ...
      if (manager && typeof manager.registerGGBlabHandler === 'function') {
        try {
          const unregisterFromManager = manager.registerGGBlabHandler(id, (commOp: any, msg: any) => {
            try { } catch (e) { console.warn('Error delegating jupyter.ggblab to manager', e); }
          });
          registry.set(id, () => { unregisterFromManager && unregisterFromManager(); });
          return;
        } catch (e) {
      ...
      try {
        kc.registerCommTarget('jupyter.ggblab', (commOp: any, msg: any) => {
          try {
            ...
            commOp.onMsg = (m: any) => { dbg('jupyter.ggblab message', { kernelId: id, m }); };
          } catch (e) {
      ...
      const unregister = () => {
        try { if (typeof (kc as any).unregisterCommTarget === 'function') { (kc as any).unregisterCommTarget('jupyter.ggblab'); } } catch (e) { console.warn('Error while unregistering jupyter.ggblab', e); }
      };
  ...
  const unregisterKernel = (id: string) => {
    ...
    if (fn) {
      try { fn(); } catch (e) { console.warn('Error during unregister for kernel', id, e); }
      registry.delete(id);
    }
  };
  ...
  const onRunningChanged = async () => {
    try {
      ...
      Array.from(registry.keys()).forEach(id => { if (!currentIds.has(id)) { unregisterKernel(id); } });
    } catch (e) {
  ...
  try {
    if (ENABLE_RUNNING_CHANGED) {
      try {
        if (app && app.serviceManager && app.serviceManager.sessions && typeof app.serviceManager.sessions.runningChanged === 'object' && typeof app.serviceManager.sessions.runningChanged.connect === 'function') {
  ...
  return () => {
    try {
      if (app && app.serviceManager && app.serviceManager.sessions && typeof app.serviceManager.sessions.runningChanged === 'object' && typeof app.serviceManager.sessions.runningChanged.disconnect === 'function') {
      ...
      Array.from(registry.keys()).forEach(k => { const fn = registry.get(k); if (fn) { fn(); } });
      registry.clear();
    } catch (e) {
